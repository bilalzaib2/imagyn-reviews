...
export function createGeminiProvider(): AiProvider {
  return {
    ...
    async generateBrandSuggestion(request: AiBrandSuggestionRequest): Promise<AiBrandSuggestionResult> {
      ...
      if (!apiKey) {
        throw new AiProviderError(
          "GEMINI_API_KEY is not configured. Set it in the environment to enable AI brand suggestions.",
          "gemini",
        );
      }
      ...
      const response = await fetch(
        `https://generativelanguage.googleapis.com/v1beta/models/${model}:generateContent?key=${apiKey}`,
        {
          ...
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            systemInstruction: { parts: [{ text: buildBrandSuggestionSystemPrompt() }] },
            contents: [{ role: "user", parts: [{ text: buildBrandSuggestionUserPrompt(request) }] }],
            generationConfig: {
              temperature: 0.5,
              responseMimeType: "application/json",
            },
